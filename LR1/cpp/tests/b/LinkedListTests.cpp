...
void testAddBefore() {
    ...
    addHead(list, "10");
    addHead(list, "20");
    bool added = addBefore(list, "10", "15");
    assert(added && list.head->next->value == "15");
    added = addBefore(list, "20", "5");
...
void testAddAfter() {
    ...
    addHead(list, "10");
    addHead(list, "20");
    bool added = addAfter(list, "10", "15");
...
void testStress() {
    ...
    assert(cur->value == std::to_string(N - 1));
    assert(count == N);
    Node* middle = getNode(list, N/2);
    assert(middle && middle->value == std::to_string(N/2));
    for (int i = 0; i < N/2; i++) {
        bool removed = removeValue(list, std::to_string(i));
        assert(removed);
    }
    cur = list.head;
    int expected = N/2;
    while (cur) {
        assert(cur->value == std::to_string(expected));
        expected++;
        cur = cur->next;
    }
    assert(expected == N);
